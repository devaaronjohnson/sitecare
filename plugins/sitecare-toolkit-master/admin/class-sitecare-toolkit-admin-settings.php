...
/**
 * Main Class file for `SiteCare_Toolkit_Settings`
 *
...
if ( ! class_exists( 'SiteCare_Toolkit_Settings' ) ) :
	...
	class SiteCare_Toolkit_Settings {
		...
		function callback_checkbox( $args ) {
			...
			$html  = '<fieldset>';
			$html .= sprintf( '<label for="sitecare-toolkit-%1$s[%2$s]">', $args['section'], $args['id'] );
			$html .= sprintf( '<input type="hidden" name="%1$s[%2$s]" value="off" />', $args['section'], $args['id'] );
			$html .= sprintf( '<input type="checkbox" class="checkbox" id="sitecare-toolkit-%1$s[%2$s]" name="%1$s[%2$s]" value="on" %3$s />', $args['section'], $args['id'], checked( $value, 'on', false ) );
		...
		function callback_multicheck( $args ) {
			...
			foreach ( $args['options'] as $key => $label ) {
				...
				$html   .= sprintf( '<label for="sitecare-toolkit-%1$s[%2$s][%3$s]">', $args['section'], $args['id'], $key );
				$html   .= sprintf( '<input type="checkbox" class="checkbox" id="sitecare-toolkit-%1$s[%2$s][%3$s]" name="%1$s[%2$s][%3$s]" value="%3$s" %4$s />', $args['section'], $args['id'], $key, checked( $checked, $key, false ) );
		...
		function callback_radio( $args ) {
			...
			foreach ( $args['options'] as $key => $label ) {
				$html .= sprintf( '<label for="sitecare-toolkit-%1$s[%2$s][%3$s]">', $args['section'], $args['id'], $key );
				$html .= sprintf( '<input type="radio" class="radio" id="sitecare-toolkit-%1$s[%2$s][%3$s]" name="%1$s[%2$s]" value="%3$s" %4$s />', $args['section'], $args['id'], $key, checked( $value, $key, false ) );
				$html .= sprintf( '%1$s</label><br>', $label );
			}
			$html .= $this->get_field_description( $args );
			$html .= '</fieldset>';
		...
		public function admin_menu() {
			add_options_page(
				__( 'SiteCare Toolkit', 'sitecare-toolkit' ),
				__( 'SiteCare Toolkit', 'sitecare-toolkit' ),
				'manage_options',
				'sitecare_toolkit_admin_settings',
				array( $this, 'plugin_page' )
			);
		}

		public function plugin_page() {
			echo '<div class="wrap sitecare-toolkit">';
			echo '<h1>' . esc_attr__( 'SiteCare Toolkit Settings', 'sitecare-toolkit' ) . ' <span style="font-size:50%;">v' . SITECARE_TOOLKIT_VERSION . '</span></h1>';
	...
	} // SiteCare_Toolkit_Settings ended.
